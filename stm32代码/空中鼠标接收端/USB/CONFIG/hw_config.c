...
{
  u8 key_Buffer[5] = {0, 0, 0, 0, 0};
	u8 mou_Buffer[8] = {0, 0, 0, 0, 0, 0, 0, 0};
	  UserToPMABufferCopy(key_Buffer, GetEPTxAddr(ENDP1), 8);
		...
		mou_Buffer[3]=buf2;
		mou_Buffer[4]=buf3;
		key_Buffer[2]=buf4;//PrtScn

		
		UserToPMABufferCopy(key_Buffer, GetEPTxAddr(ENDP1), 8);
		SetEPTxValid(ENDP1);
		UserToPMABufferCopy(mou_Buffer, GetEPTxAddr(ENDP2), 5);
		SetEPTxValid(ENDP2);
}
